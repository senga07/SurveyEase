...
### 1. 对话流程设计
- **状态机模式**：使用 LangGraph 构建对话状态机
- **条件分支**：根据用户回答决定下一步行动
- **中断机制**：支持用户主动输入和系统等待

### 2. 智能记忆管理
- **LangMem 集成**：使用 LangMem 进行智能记忆管理
- **消息压缩**：在节点切换时压缩历史消息
- **语义提取**：自动提取关键事实和洞察
...
### 4. 实时通信
- **流式处理**：使用 FastAPI 的流式响应
- **WebSocket**：前端实时接收 AI 回复
- **状态同步**：确保前后端状态一致

### 5. 数据持久化
- **检查点机制**：使用 LangGraph 检查点保存对话状态
- **模板存储**：JSON 文件存储调研模板
- **日志记录**：完整的操作日志和错误追踪
...
### 嵌入模型配置
- **阿里云百炼**：`dashscope:text-embedding-v2`
...
### 添加新的 LLM 提供商
1. 在 `llm_provider/` 目录下创建新的提供商类
2. 实现 `get_llm()` 方法
3. 在 `service_manager.py` 中注册新提供商

### 自定义调研步骤
1. 修改 `survey_template.json` 文件
2. 添加新的步骤和问题
3. 更新系统提示以指导 AI 行为

### 扩展记忆管理
1. 在 `memory/` 目录下添加新的记忆类型
2. 集成到 `SurveyGraph` 中
3. 配置记忆压缩策略
